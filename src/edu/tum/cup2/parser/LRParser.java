...
import edu.tum.cup2.parser.exceptions.*;
...
public class LRParser implements Serializable
{
	...
	public synchronized Object parse(Scanner input, boolean saveTokens, Object... initArgs)
		...
		try {
			...
			while (true) {
				...
				LRAction action = table.getActionTable().get(currentState,
				...
				if (action instanceof Shift) {
					...
					if (DEBUG)
						System.out.println("S: to state " + shift.getState().getID() + " ("
								+ currentToken.getSymbol() + ")"); // TEST
					...
					else
					{
						...
						valueStack.push(currentToken.hasValue() ? currentToken.getValue()
								: NoValue);
					}
	        ...
	        int numTokensPopped = 0;
					...
					if (DEBUG)
						System.out.println("R: " + production.toString() + " ("
								+ currentToken.getSymbol() + ")"); // TEST
	         ...
	         **/
					...
					else
					{
						...
						if (reduceAction != null) {
							newValue = ActionPerformer.perform(
							    reduceAction,
							    valueStack,
									production.getRHSSizeWithoutEpsilon()+(
									    (production.getLHS() instanceof AuxiliaryLHS4SemanticShiftAction)?
									    ((AuxiliaryLHS4SemanticShiftAction)production.getLHS()).numPrecedingSymbolsNotEpsilon:0
									)
							);
						}
						// for each symbol in the right-hand side of rule, remove one state from
					...
					// reduced
					LRParserState newState = table.getGotoTable().get(stack.peek(),
							production.getLHS());
					if (newState instanceof ErrorState) {
					  ...
					  s.beginLine     = lastBeginInfo.beginLine;
						stack.push(s);
						if (DEBUG)
							System.out.println("G: to state " + newState.getID() + " ("
									+ currentToken.getSymbol() + ")"); // TEST
					}
	          ...
	          tokenCountStack.push(numTokensPopped);
				...
				// ACTION : ACCEPT
					...
					*/
					...
					if (lastError_sync_size > 0) {
						...
						if (DEBUG)
							System.out
								.println("FINISHED, BUT DRY RUN STILL ACTIVE - now performing actions on valueStack ..");
				...
				// ACTION : UNKNOWN
					...
					table.getParserInterface().exit(this);
					throw new ErrorActionException(currentState, currentToken
							.getSymbol());
				}
			}
		}catch (Exception e) {
  ...
  private void startPhraseBasedErrorRecovery() throws IOException, EndOfInputstreamException, MissingErrorRecoveryException
  {
    ...
    if (lastError_sync_size > 0) {
      ...
      if (DEBUG)
        System.out.println("Previous error is still unsynchronized (sync:"
          + lastError_sync_size + ")! Combining errors...");
      if (saveTokens)
      {
      ...
      try {
        ...
        if (DEBUG) {
          ...
          System.out.println("  with popped values        : "
              + popped_values);
          System.out.println("  and popped tokens      : "
              + popped_tokens);
        }
      } catch (java.util.EmptyStackException e) {
		// notify observers about non-recoverable error.
		ErrorInformation errInf = new ErrorInformation(crash_token, false,
    ...
    while (
    	table.getActionTable().get(
    		handleState,
    		currentToken.getSymbol()
    	) instanceof ErrorAction
    	||
      ...
      if (currentToken.getSymbol() == SpecialTerminals.EndOfInputStream) {
        ...
        notifyObserversAbout(
          new ErrorInformation(
            ...
            read_tokens.toArray(new ScannerToken[] {}),
            expected_terminals.toArray(new Terminal[]{}),
            beginLine, beginColumn, currentToken.getLine(), currentToken.getColumn()
          )
        );
  ...
  private void dryRun_doReturnToNormal() {
    ...
    int tokenIndex = 0;
    notifyObserversAbout(lastErrorInformation);
    ...
    for(LRAction action2:dryRun_savedActions)
    {
      if (action2 instanceof Shift) {
        ...
        valueStack.push(
          ...
          parsedTokens.get(parsedTokensOffset+tokenIndex).getValue():NoValue
        );
        // "read" next token
        tokenIndex++;
      }
        ...
        if (reduceAction != null) {
          newValue = ActionPerformer.perform(
              ...
              reduce.getProduction().getRHSSizeWithoutEpsilon()+(
                  (reduce.getProduction().getLHS() instanceof AuxiliaryLHS4SemanticShiftAction)?
                  ((AuxiliaryLHS4SemanticShiftAction)reduce.getProduction().getLHS()).numPrecedingSymbolsNotEpsilon:0
              )
          );
        }
	...
	private void notifyObserversAbout(ErrorInformation e) {
